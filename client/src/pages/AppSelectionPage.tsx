    ...
    <div className="min-h-screen pb-28 dark:bg-gray-900">
      ...
      <div className={`bg-gradient-to-br ${config.gradient} text-white relative overflow-hidden`}>
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute top-1/4 left-1/3 w-64 h-64 bg-white/5 rounded-full blur-2xl"></div>
          {/* Grid pattern */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center text-4xl sm:text-5xl shadow-xl border border-white/30">
                  {config.icon}
                </div>
                <div>
                  <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold leading-tight">{t(config.titleKey)}</h1>
                  <p className="text-white/80 text-sm sm:text-base mt-1">{t(config.subtitleKey)}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {/* Total Apps */}
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20 hover:bg-white/20 transition-all hover:scale-105 cursor-default">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      <p className="text-xl sm:text-2xl font-bold">{totalApps}</p>
                      <p className="text-[10px] sm:text-xs text-white/70">{t('appSelection.stats.totalApps')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20 hover:bg-white/20 transition-all hover:scale-105 cursor-default">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${selectionCount > 0 ? 'bg-green-400/40' : 'bg-white/20'}`}>
                      <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-xl sm:text-2xl font-bold">{selectionCount}</p>
                      <p className="text-[10px] sm:text-xs text-white/70">{t('appSelection.stats.selected')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20 hover:bg-white/20 transition-all hover:scale-105 cursor-default">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${favoritesCount > 0 ? 'bg-rose-400/40' : 'bg-white/20'}`}>
                      <svg className="w-4 h-4 sm:w-5 sm:h-5" fill={favoritesCount > 0 ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-xl sm:text-2xl font-bold">{favoritesCount}</p>
                      <p className="text-[10px] sm:text-xs text-white/70">{t('appSelection.filter.favorites')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20 hover:bg-white/20 transition-all hover:scale-105 cursor-default">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] sm:text-xs text-white/70">{t('appSelection.filter.category')}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                ...
                <Link
                  to="/summary"
                  className="flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 border border-white/30 rounded-xl font-medium text-sm transition-all"
                >
                  ...
                  {t('summary.title')}
                  {selectionCount > 0 && (
                    <span className="bg-white/30 px-2 py-0.5 rounded-full text-xs">{selectionCount}</span>
                  )}
                </Link>
                {hasActiveFilters && (
                  <button
                    onClick={clearAllFilters}
                    className="flex items-center gap-2 px-4 py-2 bg-red-500/20 hover:bg-red-500/30 border border-red-300/30 rounded-xl font-medium text-sm transition-all"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Right Column: Search + Category Quick Select */}
            <div className="space-y-4">
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-4 pr-10 py-3 bg-white rounded-xl text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-white/50 border-0 shadow-lg"
                  />
                  {searchQuery && (
                    <button
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                <label className="block text-sm font-medium text-white/80 mb-2">
                  ...
                  {t('appSelection.filter.category')}
                </label>
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    {t('appSelection.filter.allCategories')}
                  </button>
              ...
              {categories.length > 0 && categories[0].apps.length > 0 && (
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                  ...
                  <div className="space-y-2">
                    {categories.flatMap(cat => cat.apps).slice(0, 3).map((app) => (
                      <Link
                        key={app.id}
                        to={`/apps/${app.id}`}
                        className="flex items-center gap-3 p-2 rounded-xl bg-white/10 hover:bg-white/20 transition-all group"
                      >
                        <img
                          src={app.iconUrl}
                          alt={app.name}
                          ...
                          onError={(e) => {
                            (e.target as HTMLImageElement).src =
                              'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>';
                          }}
                        />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate group-hover:text-white/90">{app.name}</p>
                          <p className="text-xs text-white/60 truncate">{app.description.slice(0, 40)}...</p>
                        </div>
